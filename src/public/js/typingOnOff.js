...
$(document).ready(function(){
    // Lắng nghe su kien khi focus
    socket.on("response-user-is-typing",function(response){
        let messageTyping=`<div class="buble you buble-typing-gif">
            <img src="/images/chat/typing.gif"/>
        </div>`;

        if(response.currentGroupId){
            if(response.currentUserId !==$("#dropdown-navbar-user").data("uid")){
                let checkTyping=$(`.chat[data-chat=${response.currentGroupId}]`).find("div.buble-typing-gif");
            ...
            let checkTyping=$(`.chat[data-chat=${response.currentUserId}]`).find("div.buble-typing-gif");
            if(checkTyping.length){
                return false;
            }
     ...
     // Lắng nghe su kien khi tăt focus
     socket.on("response-user-is-not-typing", function(response){
        if(response.currentGroupId){
            if(response.currentUserId !==$("#dropdown-navbar-user").data("uid")){
                $(`.chat[data-chat=${response.currentGroupId}]`).find("div.buble-typing-gif").remove();
                nineScrollRight(response.currentGroupId);
            }
        }
        else{
            $(`.chat[data-chat=${response.currentUserId}]`).find("div.buble-typing-gif").remove();
            nineScrollRight(response.currentUserId);
        }
     })
});